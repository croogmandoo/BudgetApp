...
{:else}
  <div class="flex min-h-screen bg-surface-0">
    <!-- ── Sidebar (md+) ── -->
    <aside class="hidden md:flex md:w-56 md:shrink-0 md:flex-col bg-surface-1 border-r border-surface-3">
      <!-- Logo -->
      <div class="px-5 pt-6 pb-4">
        <a href="/" class="text-lg font-display font-semibold text-ink-primary tracking-tight leading-none">
          BudgetApp
        </a>
      ...
      <nav class="flex-1 px-2 space-y-0.5">
        {#each nav as item (item.href)}
          ...
          >
            {#if active}
              <span class="absolute left-0 top-1.5 bottom-1.5 w-0.5 rounded-r-full bg-accent"></span>
            {/if}
            <svg
      ...
      <div class="border-t border-surface-3 px-2 py-3 space-y-0.5">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" class="shrink-0" aria-hidden="true">
            <path d={icons.settings} />
          </svg>
          <span>Settings</span>
        </a>
        <div class="flex items-center gap-3 px-3 py-2 text-sm">
          <span class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-accent/20 text-[11px] font-semibold text-accent">C</span>
  ...
  <nav class="fixed bottom-0 left-0 right-0 z-50 flex border-t border-surface-3 bg-surface-1 md:hidden" aria-label="Main navigation">
    {#each nav as item (item.href)}
      {@const active = isActive(item)}
      <a
        href={item.href}
      ...
      >
        ...
        <span class="text-[10px] leading-none">{item.label.length > 6 ? item.label.slice(0, 5) + '…' : item.label}</span>
      </a>
    {/each}
  </nav>
{/if}

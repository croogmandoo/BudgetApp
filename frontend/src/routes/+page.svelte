<script lang="ts">
  ...
  const transactions = [
    { date: 'Apr 22', payee: 'Whole Foods Market', category: 'Groceries', amount: '−$89.43', neg: true },
    ...
    { date: 'Apr 19', payee: 'Shell Gas Station', category: 'Auto · Fuel', amount: '−$67.80', neg: true },
    { date: 'Apr 18', payee: 'Rogers Wireless', category: 'Phone', amount: '−$95.00', neg: true },
    { date: 'Apr 17', payee: 'LCBO', category: null, amount: '−$34.50', neg: true, uncategorized: true },
...
<div class="px-5 py-6 space-y-6 max-w-4xl">
  ...
  <div class="flex items-end justify-between">
    ...
    <div class="flex items-center gap-2">
      <button class="btn-ghost text-xs px-3 py-1.5">Import</button>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    {#each stats as s}
      <div class="card px-4 py-3">
        ...
        <p class="num text-xl font-medium {s.accent ? 'text-accent' : s.warn ? 'text-caution' : 'text-ink-primary'}">
          {s.value}
        </p>
        {#if s.trend}
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-surface-3">
            <th class="px-4 py-2 text-left text-[11px] font-medium uppercase tracking-wider text-ink-muted">Date</th>
            <th class="px-4 py-2 text-left text-[11px] font-medium uppercase tracking-wider text-ink-muted">Payee</th>
            <th class="hidden px-4 py-2 text-left text-[11px] font-medium uppercase tracking-wider text-ink-muted sm:table-cell">Category</th>
            <th class="px-4 py-2 text-right text-[11px] font-medium uppercase tracking-wider text-ink-muted">Amount</th>
          </tr>
        </thead>
        <tbody>
          {#each transactions as tx}
            <tr class="border-b border-surface-3/50 transition-colors duration-100 hover:bg-surface-2 last:border-0">
              ...
              <td class="px-4 py-2.5 text-right">
                <span class="num text-[13px] {tx.pos ? 'num-pos' : tx.neg ? 'num-neg' : ''}">{tx.amount}</span>
              </td>
            </tr>
          {/each}
        </tbody>
